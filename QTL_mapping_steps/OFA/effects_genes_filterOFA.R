...
pheno.do <- read.csv(
  file.path(DATA_DIR, "OFA_residuals_07012020.csv"),
...
message("LOD range BEFORE: ",
        paste(range(out, na.rm = TRUE), collapse = " – "))

out_filt <- out
out_filt[out_filt < LOD_cutoff] <- NA

message("LOD range AFTER:  ",
        paste(range(out_filt, na.rm = TRUE), collapse = " – "))
